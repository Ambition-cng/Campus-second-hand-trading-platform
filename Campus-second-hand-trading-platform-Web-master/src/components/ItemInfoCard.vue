  ...
  <el-card :body-style="{ padding: '10px' }" style="margin: 20px">
    ...
    <div style="padding: 14px">
      ...
      <div class="bottom clearfix">
        <el-button type="text" class="button" @click="DeleteItem"
          >删除发布商品</el-button
        >
...
export default {
  ...
  methods: {
    DeleteItem: function () {
      ...
      this.$axios
        .delete(`api/item/deleteItem?itemId=${that.itemInfo.itemid}`)
        .then((res) => {
          console.log(res);
    ...
    AddComment: function () {
      ...
      this.$axios.post("api/comment/item", params).then(function (res) {
        // 注册成功
        if (res.data.code == 1) {
          that.$message({
            message: "恭喜你，评论发布成功!",
      ...
      this.$axios.delete(`api/comment/deleteItemComment?commentId=${commentid}`).then((res) => {
        // 获取成功
        if (res.data.code == 1) {
            that.$message({
              message: "恭喜你!控评成功！",
    ...
    GetAllComments: function () {
      ...
      this.$axios.get(`api/comment/getItemCommentById?itemId=${that.itemInfo.itemid}`).then((res) => {
        // 获取成功
        if (res.data.code == 1) {
